...
CallCore::CallCore(const std::shared_ptr<linphone::Call> &call) : QObject(nullptr) {
	...
	mDir = ZuppiEnums::fromZuppi(call->getDir());
	...
	mRemoteVideoEnabled =
	    videoDirection == linphone::MediaDirection::SendOnly || videoDirection == linphone::MediaDirection::SendRecv;
	mState = ZuppiEnums::fromZuppi(call->getState());
	...
	mStatus = ZuppiEnums::fromZuppi(call->getCallLog()->getStatus());

	mTransferState = ZuppiEnums::fromZuppi(call->getTransferState());
	...
	mEncryption = ZuppiEnums::fromZuppi(call->getParams()->getMediaEncryption());
	auto tokenVerified = call->getAuthenticationTokenVerified();
	mIsSecured = (mEncryption == ZuppiEnums::MediaEncryption::Zrtp && tokenVerified) ||
	             mEncryption == ZuppiEnums::MediaEncryption::Srtp ||
	             mEncryption == ZuppiEnums::MediaEncryption::Dtls;
	if (mEncryption == ZuppiEnums::MediaEncryption::Zrtp) {
	...
	mPaused = mState == ZuppiEnums::CallState::Pausing || mState == ZuppiEnums::CallState::Paused ||
	          mState == ZuppiEnums::CallState::PausedByRemote;
	...
	mRecordable = mState == ZuppiEnums::CallState::StreamsRunning;
...
void CallCore::setSelf(QSharedPointer<CallCore> me) {
	...
	mCallModelConnection->makeConnectToModel(&CallModel::stateChanged, [this](std::shared_ptr<linphone::Call> call,
		...
		mCallModelConnection->invokeToCore([this, state, message]() { setState(ZuppiEnums::fromZuppi(state)); });
	});
	mCallModelConnection->makeConnectToModel(&CallModel::statusChanged, [this](linphone::Call::Status status) {
		mCallModelConnection->invokeToCore([this, status]() { setStatus(ZuppiEnums::fromZuppi(status)); });
	});
	...
	mCallModelConnection->makeConnectToModel(
	    ...
	    [this](const std::shared_ptr<linphone::Call> &call, linphone::Call::State state) {
		    mCallModelConnection->invokeToCore(
		        [this, state]() { setTransferState(ZuppiEnums::fromZuppi(state)); });
	    });
	mCallModelConnection->makeConnectToModel(
	    &CallModel::encryptionChanged,
	    [this](const std::shared_ptr<linphone::Call> &call, bool on, const std::string &authenticationToken) {
		    auto encryption = ZuppiEnums::fromZuppi(call->getCurrentParams()->getMediaEncryption());
	...
	mCallModelConnection->makeConnectToModel(
	    &CallModel::conferenceVideoLayoutChanged, [this](ZuppiEnums::ConferenceLayout layout) {
		    mCallModelConnection->invokeToCore([this, layout]() { setConferenceVideoLayout(layout); });
	    });
	mCallModelConnection->makeConnectToCore(
	    &CallCore::lSetConferenceVideoLayout, [this](ZuppiEnums::ConferenceLayout layout) {
...
ZuppiEnums::CallStatus CallCore::getStatus() const {
	return mStatus;
}

void CallCore::setStatus(ZuppiEnums::CallStatus status) {
...
ZuppiEnums::CallDir CallCore::getDir() const {
	return mDir;
}

void CallCore::setDir(ZuppiEnums::CallDir dir) {
	mustBeInMainThread(log().arg(Q_FUNC_INFO));
	if (mDir != dir) {
		mDir = dir;
...
ZuppiEnums::CallState CallCore::getState() const {
	return mState;
}

void CallCore::setState(ZuppiEnums::CallState state) {
	mustBeInMainThread(log().arg(Q_FUNC_INFO));
	if (mState != state) {
		mState = state;
...
ZuppiEnums::MediaEncryption CallCore::getEncryption() const {
	return mEncryption;
}

QString CallCore::getEncryptionString() const {
	return ZuppiEnums::toString(mEncryption);
}

void CallCore::setEncryption(ZuppiEnums::MediaEncryption encryption) {
...
ZuppiEnums::CallState CallCore::getTransferState() const {
	return mTransferState;
}

void CallCore::setTransferState(ZuppiEnums::CallState state) {
...
ZuppiEnums::ConferenceLayout CallCore::getConferenceVideoLayout() const {
...
void CallCore::setConferenceVideoLayout(ZuppiEnums::ConferenceLayout layout) {
	mustBeInMainThread(log().arg(Q_FUNC_INFO));
...
void CallCore::findRemoteLdapFriend(QSharedPointer<CallCore> me) {
	...
	mLdapMagicSearchModel->search(mRemoteAddress, (int)ZuppiEnums::MagicSearchSource::LdapServers,
	                              ZuppiEnums::MagicSearchAggregation::Friend, -1);
}
