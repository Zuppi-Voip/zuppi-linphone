			    ...
			    if (method == linphone::AuthMethod::HttpDigest) {
				    ...
				    connect(obj, &AuthenticationDialog::result, this, [this, obj, authInfo, core](QString password) {
					    mCoreModelConnection->invokeToModel([this, core, authInfo, password] {
						    mustBeInZuppiThread("[App] reauthenticate");
						    if (password.isEmpty()) {
...
QThread *App::getZuppiThread() {
	return App::getInstance()->mZuppiThread;
}
...
void App::init() {
	...
	if (!mZuppiThread->isRunning()) {
		lInfo() << log().arg("Starting Thread");
		mZuppiThread->start();
		while (!mZuppiThread->getThreadId()) // Wait for running thread
			processEvents();
	}
	...
	QMetaObject::invokeMethod(
	    ...
	    [this, settings = mSettings]() mutable {
		    ...
		    QMetaObject::invokeMethod(App::getInstance()->thread(), [this, settings, coreStarted] {
			    ...
			    const QUrl url("qrc:/qt/qml/Zuppi/view/Page/Window/Main/MainWindow.qml");
			    QObject::connect(
...
QQuickWindow *App::getCallsWindow(QVariant callGui) {
	...
	if (!mCallsWindow) {
		const QUrl callUrl("qrc:/qt/qml/Zuppi/view/Page/Window/Call/CallsWindow.qml");
