...
QVariant createFriendDevice(const QString &name, const QString &address, ZuppiEnums::SecurityLevel level) {
	QVariantMap map;
...
FriendCore::FriendCore(const std::shared_ptr<linphone::Friend> &contact, bool isStored) : QObject(nullptr) {
	...
	if (contact) {
		...
		mConsolidatedPresence = ZuppiEnums::fromZuppi(contact->getConsolidatedPresence());
		...
		for (auto &device : devices) {
			mDeviceList.append(createFriendDevice(Utils::coreStringToAppString(device->getDisplayName()),
			                                      ...
			                                      ZuppiEnums::fromZuppi(device->getSecurityLevel())));
		}
	...
	if (me) {
		if (mFriendModel) {
			...
			mFriendModelConnection->makeConnectToModel(
			    ...
			    [this](ZuppiEnums::ConsolidatedPresence consolidatedPresence, QDateTime presenceTimestamp) {
...
void FriendCore::updateVerifiedDevicesCount() {
	...
	for (auto &device : mDeviceList) {
		...
		if (map["securityLevel"].value<ZuppiEnums::SecurityLevel>() ==
		    ZuppiEnums::SecurityLevel::EndToEndEncryptedAndVerified)
...
ZuppiEnums::SecurityLevel FriendCore::getSecurityLevelForAddress(const QString &address) const {
	for (auto &device : mDeviceList) {
		auto map = device.toMap();
		if (map["address"].toString() == address) {
			return map["securityLevel"].value<ZuppiEnums::SecurityLevel>();
		}
	}
	return ZuppiEnums::SecurityLevel::None;
}
...
ZuppiEnums::ConsolidatedPresence FriendCore::getConsolidatedPresence() const {
	return mConsolidatedPresence;
}

void FriendCore::setConsolidatedPresence(ZuppiEnums::ConsolidatedPresence presence) {
...
void FriendCore::writeIntoModel(std::shared_ptr<FriendModel> model) const {
	mustBeInZuppiThread(QString("[") + gClassName + "] " + Q_FUNC_INFO);
