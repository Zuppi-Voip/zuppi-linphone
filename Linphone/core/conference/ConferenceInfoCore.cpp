...
ConferenceInfoCore::create(std::shared_ptr<linphone::ConferenceInfo> conferenceInfo) {
	...
	if (isInZuppiThread()) sharedPointer->moveToThread(App::getInstance()->thread());
...
ConferenceInfoCore::ConferenceInfoCore(std::shared_ptr<linphone::ConferenceInfo> conferenceInfo, QObject *parent)
	...
	if (conferenceInfo) {
		...
		for (auto item : conferenceInfo->getParticipantInfos()) {
			...
			participant["role"] = (int)ZuppiEnums::fromZuppi(item->getRole());
			mParticipants.append(participant);
		}
		mConferenceInfoState = ZuppiEnums::fromZuppi(conferenceInfo->getState());
	} else {
		...
		if (mConferenceInfoModel) {
			...
			mConfInfoModelConnection->makeConnectToModel(
			    &ConferenceInfoModel::schedulerStateChanged, [this](linphone::ConferenceScheduler::State state) {
				    ...
				    mConfInfoModelConnection->invokeToCore([this, state = ZuppiEnums::fromZuppi(state),
				                                            infoState = ZuppiEnums::fromZuppi(confInfoState),
				                                            uri] {
					    setConferenceSchedulerState(state);
					    setConferenceInfoState(infoState);
					    if (state == ZuppiEnums::ConferenceSchedulerState::Ready) {
						    setUri(uri);
			...
			mConfInfoModelConnection->makeConnectToModel(
			    &ConferenceInfoModel::infoStateChanged, [this](linphone::ConferenceInfo::State state) {
				    ...
				    mConfInfoModelConnection->invokeToCore([this, infoState = ZuppiEnums::fromZuppi(state), uri] {
					    setConferenceInfoState(infoState);
				    });
			    });
			mConfInfoModelConnection->makeConnectToModel(
...
void ConferenceInfoCore::addParticipant(const QString &address) {
	...
	participant["role"] = (int)ZuppiEnums::ParticipantRole::Listener;
	mParticipants.append(participant);
...
ZuppiEnums::ConferenceInfoState ConferenceInfoCore::getConferenceInfoState() const {
	return mConferenceInfoState;
}

ZuppiEnums::ConferenceSchedulerState ConferenceInfoCore::getConferenceSchedulerState() const {
...
void ConferenceInfoCore::setConferenceInfoState(ZuppiEnums::ConferenceInfoState state) {
...
void ConferenceInfoCore::setConferenceSchedulerState(ZuppiEnums::ConferenceSchedulerState state) {
...
void ConferenceInfoCore::writeFromModel(const std::shared_ptr<ConferenceInfoModel> &model) {
	mustBeInZuppiThread(getClassName() + "::writeFromModel()");
...
void ConferenceInfoCore::writeIntoModel(std::shared_ptr<ConferenceInfoModel> model) {
	mustBeInZuppiThread(getClassName() + "::writeIntoModel()");
...
void ConferenceInfoCore::save() {
	...
	if (mConferenceInfoModel) {
		mConfInfoModelConnection->invokeToModel([this, thisCopy]() { // Copy values to avoid concurrency
			mustBeInZuppiThread(getClassName() + "::save()");
