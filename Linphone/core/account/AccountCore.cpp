...
AccountCore::AccountCore(const std::shared_ptr<linphone::Account> &account) : QObject(nullptr) {
	...
	mRegistrationState = ZuppiEnums::fromZuppi(account->getState());
	...
	mTransports << "TCP"
	            ...
	            << "TLS"
	            << "DTLS";
	mTransport = ZuppiEnums::toString(ZuppiEnums::fromZuppi(params->getTransport()));
	mServerAddress =
...
void AccountCore::setSelf(QSharedPointer<AccountCore> me) {
	...
	mAccountModelConnection->makeConnectToModel(&AccountModel::transportChanged, [this](linphone::TransportType value) {
		mAccountModelConnection->invokeToCore(
		    [this, value]() { onTransportChanged(ZuppiEnums::toString(ZuppiEnums::fromZuppi(value))); });
	});
	...
	mAccountModelConnection->makeConnectToCore(&AccountCore::lSetTransport, [this](QString value) {
		ZuppiEnums::TransportType transport;
		ZuppiEnums::fromString(value, &transport);
		mAccountModelConnection->invokeToModel(
		    [this, value, transport]() { mAccountModel->setTransport(ZuppiEnums::toZuppi(transport)); });
	});
...
ZuppiEnums::RegistrationState AccountCore::getRegistrationState() const {
...
void AccountCore::onRegistrationStateChanged(const std::shared_ptr<linphone::Account> &account,
	...
	mRegistrationState = ZuppiEnums::fromZuppi(state);
...
QString AccountCore::getHumanReadableRegistrationState() const {
	switch (mRegistrationState) {
		case ZuppiEnums::RegistrationState::Ok:
			return tr("Connecté");
		case ZuppiEnums::RegistrationState::Refreshing:
			return tr("En cours de rafraîchissement…");
		case ZuppiEnums::RegistrationState::Progress:
			return tr("En cours de connexion…");
		case ZuppiEnums::RegistrationState::Failed:
			return tr("Erreur");
		case ZuppiEnums::RegistrationState::None:
		case ZuppiEnums::RegistrationState::Cleared:
			return tr("Désactivé");
		default:
			return " ";
	}
