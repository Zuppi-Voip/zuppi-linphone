...
void MagicSearchModel::search(QString filter,
                              int sourceFlags,
                              ZuppiEnums::MagicSearchAggregation aggregation,
                              int maxResults) {
	mLastSearch = filter;
	setMaxResults(maxResults);
	if (filter == "" || filter == "*") {
		if (((sourceFlags & (int)ZuppiEnums::MagicSearchSource::LdapServers) > 0) &&
		    !SettingsModel::getInstance()->getSyncLdapContacts())
			sourceFlags &= ~(int)ZuppiEnums::MagicSearchSource::LdapServers;
		// For complete search, we search only on local contacts.
		// sourceFlags &= ~(int)ZuppiEnums::MagicSearchSource::CallLogs;
		// sourceFlags &= ~(int)ZuppiEnums::MagicSearchSource::ChatRooms;
		// sourceFlags &= ~(int)ZuppiEnums::MagicSearchSource::ConferencesInfo;
	}
	...
	mMonitor->getContactsListAsync(filter != "*" ? Utils::appStringToCoreString(filter) : "", "", sourceFlags,
	                               ZuppiEnums::toZuppi(aggregation));
}
...
void MagicSearchModel::onSearchResultsReceived(const std::shared_ptr<linphone::MagicSearch> &magicSearch) {
	...
	for (auto result : results) {
		...
		bool isLdap = (result->getSourceFlags() & (int)ZuppiEnums::MagicSearchSource::LdapServers) != 0;
