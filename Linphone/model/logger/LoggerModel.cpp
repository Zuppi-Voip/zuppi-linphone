...
LoggerModel::LoggerModel(QObject *parent) : QObject(parent) {
	...
	connect(QtLogger::getInstance(), &QtLogger::requestQtOnlyEnabled, this, &LoggerModel::enableQtOnly,
	        Qt::QueuedConnection);
	connect(this, &LoggerModel::linphoneLogReceived, QtLogger::getInstance(), &QtLogger::onZuppiLog,
	        Qt::QueuedConnection);
}
...
// Call from Zuppi
void LoggerModel::onZuppiLog(const std::shared_ptr<linphone::LoggingService> &,
...
void LoggerModel::init() {
	...
	connect(mListener.get(), &LoggerListener::logReceived, this, &LoggerModel::onZuppiLog);
	{
