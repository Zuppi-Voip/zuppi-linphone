...
std::shared_ptr<linphone::FriendPhoneNumber> ToolModel::makeZuppiNumber(const QString &label,
...
void ToolModel::loadDownloadedCodecs() {
	mustBeInZuppiThread(sLog().arg(Q_FUNC_INFO));
...
QString ToolModel::computeUserAgent(const std::shared_ptr<linphone::Config> &config) {
	return QStringLiteral("%1 (%2) %3 Qt/%4 ZuppiSDK")
