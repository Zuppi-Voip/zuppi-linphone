...
ZuppiEnums::ConferenceLayout CallModel::getConferenceVideoLayout() const {
	mustBeInZuppiThread(log().arg(Q_FUNC_INFO));
	return ZuppiEnums::fromZuppi(mMonitor->getParams()->getConferenceVideoLayout());
}

void CallModel::changeConferenceVideoLayout(ZuppiEnums::ConferenceLayout layout) {
	mustBeInZuppiThread(log().arg(Q_FUNC_INFO));
	auto coreManager = CoreModel::getInstance();

	// TODO : change layout for grid/active speaker in settings
	//	if (layout == ZuppiEnums::ConferenceLayout::Grid)
	...
	params->setConferenceVideoLayout(ZuppiEnums::toZuppi(layout));
	params->enableVideo(layout != ZuppiEnums::ConferenceLayout::AudioOnly);
...
void CallModel::updateConferenceVideoLayout() {
	...
	auto newLayout = ZuppiEnums::fromZuppi(callParams->getConferenceVideoLayout());
	if (!callParams->videoEnabled()) newLayout = ZuppiEnums::ConferenceLayout::AudioOnly;
	if (!mConference) newLayout = ZuppiEnums::ConferenceLayout::ActiveSpeaker;
...
ZuppiEnums::VideoSourceScreenSharingType CallModel::getVideoSourceType() const {
	auto videoSource = mMonitor->getVideoSource();
	return ZuppiEnums::fromZuppi(videoSource ? videoSource->getScreenSharingType()
