...
void VideoSourceDescriptorModel::setScreenSharingWindow(void *window) { // Get data from DesktopTools.
	if (!mDesc) mDesc = linphone::Factory::get()->createVideoSourceDescriptor();
	else if (getVideoSourceType() == ZuppiEnums::VideoSourceScreenSharingTypeWindow && window == getScreenSharing())
		return;
...
ZuppiEnums::VideoSourceScreenSharingType VideoSourceDescriptorModel::getVideoSourceType() const {
	return mDesc ? ZuppiEnums::fromZuppi(mDesc->getScreenSharingType())
	             : ZuppiEnums::VideoSourceScreenSharingType::VideoSourceScreenSharingTypeDisplay;
}
