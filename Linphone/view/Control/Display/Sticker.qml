...
Item {
	...
	property bool remoteIsPaused: participantDevice
		...
		: previewEnabled
			? callState === ZuppiEnums.CallState.Paused
			: callState === ZuppiEnums.CallState.PausedByRemote
	...
	Rectangle {
		...
		Item {
			...
			ColumnLayout {
				...
				spacing: 0
				visible: !mainItem.account && (mainItem.callState === ZuppiEnums.CallState.OutgoingInit
						|| mainItem.callState === ZuppiEnums.CallState.OutgoingProgress
						|| mainItem.callState === ZuppiEnums.CallState.OutgoingRinging
						|| mainItem.callState === ZuppiEnums.CallState.OutgoingEarlyMedia
						|| mainItem.callState === ZuppiEnums.CallState.IncomingReceived)
				BusyIndicator {
			...
			Item{
				...
				ColumnLayout{
					...
					anchors.centerIn: parent
					spacing: 0
					visible: mainItem.participantDevice && (mainItem.participantDevice.core.state == ZuppiEnums.ParticipantDeviceState.Joining || mainItem.participantDevice.core.state == ZuppiEnums.ParticipantDeviceState.Alerting) || false
					BusyIndicator {
		...
		Loader{
			...
			active: mainItem.visible && !mainItem.remoteIsPaused 
				&& mainItem.callState != ZuppiEnums.CallState.End 
				&& mainItem.callState != ZuppiEnums.CallState.Released
				&& mainItem.callState != ZuppiEnums.CallState.Paused 
				&& mainItem.callState != ZuppiEnums.CallState.PausedByRemote
