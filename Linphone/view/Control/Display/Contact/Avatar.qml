...
Loader{
	...
	property var securityLevel: securityLevelObj ? securityLevelObj.value : ZuppiEnums.SecurityLevel.None
	property bool secured: call && call.core.encryption === ZuppiEnums.MediaEncryption.Zrtp
						   ? call.core.tokenVerified
						   : contact
							 ? contact.core.devices.length != 0 && contact.core.verifiedDeviceCount === contact.core.devices.length
							 : securityLevel === ZuppiEnums.SecurityLevel.EndToEndEncrypted
	
	property bool securityBreach: securityLevel === ZuppiEnums.SecurityLevel.Unsafe
	
	property bool displayPresence: account 
								   ? account.core?.registrationState != ZuppiEnums.RegistrationState.Progress && account.core?.registrationState != ZuppiEnums.RegistrationState.Refreshing || false
								   : contact
									 ? contact.core?.consolidatedPresence != ZuppiEnums.ConsolidatedPresence.Offline || false
									 : false
		...
		Item {
			...
			StackView {
				...
				Rectangle {
					...
					z: 1
					color: account
						   ? account.core?.registrationState == ZuppiEnums.RegistrationState.Ok
							 ? DefaultStyle.success_500main
							 : account.core?.registrationState == ZuppiEnums.RegistrationState.Cleared || account.core?.registrationState == ZuppiEnums.RegistrationState.None
							   ? DefaultStyle.warning_600
							   : account.core?.registrationState == ZuppiEnums.RegistrationState.Progress || account.core?.registrationState == ZuppiEnums.RegistrationState.Refreshing
								 ? DefaultStyle.main2_500main
								 : DefaultStyle.danger_500main
					: contact
					? contact.core.consolidatedPresence === ZuppiEnums.ConsolidatedPresence.Online
					? DefaultStyle.success_500main
					: contact.core.consolidatedPresence === ZuppiEnums.ConsolidatedPresence.Busy
					  ? DefaultStyle.warning_600
					  : contact.core.consolidatedPresence === ZuppiEnums.ConsolidatedPresence.DoNotDisturb
						? DefaultStyle.danger_500main
						: DefaultStyle.main2_500main
					: "transparent"
					border {
