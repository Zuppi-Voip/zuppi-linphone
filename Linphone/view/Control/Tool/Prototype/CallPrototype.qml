...
Window {
	...
	onCallStateChanged: {
		...
		if(callState == ZuppiEnums.CallState.Released)
		...
		ColumnLayout{
			...
			RowLayout {
				...
				Rectangle{
					...
					color: state === ZuppiEnums.RegistrationState.Ok
									? 'green'
									: state === ZuppiEnums.RegistrationState.Failed  || state === ZuppiEnums.RegistrationState.None
										? 'red'
										: 'orange'
					MouseArea{
						anchors.fill: parent
				...
				Button{
					text: 'Call'
					onClicked: {
						var address = usernameToCall.text + "@sip.zuppi.site"
			...
			Rectangle{
				...
				Layout.preferredHeight: 50
				color: call
						? call.core.state === ZuppiEnums.CallState.StreamsRunning 
							? 'green'
							: call.core.state === ZuppiEnums.CallState.Released
								? 'pink'
								: 'orange'
						: 'red'
				Rectangle{
