...
AbstractWindow {
	...
	property var callState: call ? call.core.state : ZuppiEnums.CallState.Idle
	property var transferState: call && call.core.transferState

	onCallStateChanged: {
		if (callState === ZuppiEnums.CallState.Connected) {
			...
			if(call.core.encryption === ZuppiEnums.MediaEncryption.Zrtp && !mainWindow.conference && (!call.core.tokenVerified || call.core.isMismatch)) {
				zrtpValidation.open()
			}
		}
		else if (callState === ZuppiEnums.CallState.Error || callState === ZuppiEnums.CallState.End) {
	...
	onTransferStateChanged: {
		...
		if (mainWindow.transferState === ZuppiEnums.CallState.OutgoingInit) {
			var callsWin = UtilsCpp.getCallsWindow()
			if (!callsWin) return
			callsWin.showLoadingPopup(qsTr("Transfert en cours, veuillez patienter"))
		}
		else if (mainWindow.transferState === ZuppiEnums.CallState.Error
		|| mainWindow.transferState === ZuppiEnums.CallState.End
		|| mainWindow.transferState === ZuppiEnums.CallState.Released
		|| mainWindow.transferState === ZuppiEnums.CallState.Connected) {
			var callsWin = UtilsCpp.getCallsWindow()
			callsWin.closeLoadingPopup()
			if (transferState === ZuppiEnums.CallState.Error) UtilsCpp.showInformationPopup(qsTr("Erreur"), qsTr("Le transfert d'appel a échoué"), false, mainWindow)
			else if (transferState === ZuppiEnums.CallState.Connected){
	...
	function changeLayout(layoutIndex) {
		if (layoutIndex == 0) {
			...
			call.core.lSetConferenceVideoLayout(ZuppiEnums.ConferenceLayout.Grid)
		} else if (layoutIndex == 1) {
			console.log("Set AS layout")
			call.core.lSetConferenceVideoLayout(ZuppiEnums.ConferenceLayout.ActiveSpeaker)
		} else {
			console.log("Set audio-only layout")
			call.core.lSetConferenceVideoLayout(ZuppiEnums.ConferenceLayout.AudioOnly)
		}
	}
	...
	function callEnded(call){
		if (call.core.state === ZuppiEnums.CallState.Error) {
			middleItemStackView.replace(inCallItem)
		}
		if (!callsModel.haveCall) {
	...
	Connections {
		...
		function onSecurityUpdated() {
			if (call.core.encryption === ZuppiEnums.MediaEncryption.Zrtp) {
		...
		ColumnLayout {
					...
					RowLayout {
						...
						EffectImage {
							...
							imageSource: !mainWindow.call
								? AppIcons.meeting
								: (mainWindow.callState === ZuppiEnums.CallState.End
									|| mainWindow.callState === ZuppiEnums.CallState.Released)
									? AppIcons.endCall
									: (mainWindow.callState === ZuppiEnums.CallState.Paused
									|| mainWindow.callState === ZuppiEnums.CallState.PausedByRemote)
										? AppIcons.pause
										: mainWindow.conference
											? AppIcons.usersThree
											: mainWindow.call.core.dir === ZuppiEnums.CallDir.Outgoing
												? AppIcons.arrowUpRight
												: AppIcons.arrowDownLeft
							colorizationColor: !mainWindow.call || mainWindow.call.core.paused || mainWindow.callState === ZuppiEnums.CallState.Paused
								|| mainWindow.callState === ZuppiEnums.CallState.PausedByRemote || mainWindow.callState === ZuppiEnums.CallState.End
								|| mainWindow.callState === ZuppiEnums.CallState.Released || mainWindow.conference 
						...
						ColumnLayout {
							...
							RowLayout {
								...
								Text {
									id: callStatusText
									property string remoteName: mainWindow.callState === ZuppiEnums.CallState.Connected || mainWindow.callState === ZuppiEnums.CallState.StreamsRunning
										? mainWindow.call.core.remoteName
										: qsTr("Appel %1").arg(EnumsToStringCpp.dirToString(mainWindow.call.core.dir))
									text: (mainWindow.callState === ZuppiEnums.CallState.End  || mainWindow.callState === ZuppiEnums.CallState.Released)
										? qsTr("Fin d'appel")
								...
								Rectangle {
									visible: mainWindow.call && (mainWindow.callState === ZuppiEnums.CallState.Connected
											|| mainWindow.callState === ZuppiEnums.CallState.StreamsRunning)
								...
								Text {
									...
									visible: mainWindow.callState === ZuppiEnums.CallState.Connected
											|| mainWindow.callState === ZuppiEnums.CallState.StreamsRunning
								}
								Text {
							...
							RowLayout {
								...
								visible: mainWindow.callState === ZuppiEnums.CallState.Connected || mainWindow.callState === ZuppiEnums.CallState.StreamsRunning
								BusyIndicator  {
									visible: mainWindow.call && mainWindow.callState != ZuppiEnums.CallState.Connected && mainWindow.callState != ZuppiEnums.CallState.StreamsRunning
								...
								EffectImage {
									...
									colorizationColor: mainWindow.call 
										? mainWindow.call.core.encryption === ZuppiEnums.MediaEncryption.Srtp
											? DefaultStyle.info_500_main
											: mainWindow.call.core.encryption === ZuppiEnums.MediaEncryption.Zrtp 
									...
									? mainWindow.call.core.encryption === ZuppiEnums.MediaEncryption.Srtp
										...
										? AppIcons.lockSimple
										: mainWindow.call && mainWindow.call.core.encryption === ZuppiEnums.MediaEncryption.Zrtp 
											? mainWindow.call.core.isMismatch || !mainWindow.call.core.tokenVerified
								...
								Text {
									text: mainWindow.call && mainWindow.callState === ZuppiEnums.CallState.Connected || mainWindow.callState === ZuppiEnums.CallState.StreamsRunning
									? mainWindow.call.core.encryption === ZuppiEnums.MediaEncryption.Srtp
									...
									color: mainWindow.call && mainWindow.callState === ZuppiEnums.CallState.Connected || mainWindow.callState === ZuppiEnums.CallState.StreamsRunning
									? mainWindow.call.core.encryption === ZuppiEnums.MediaEncryption.Srtp
										? DefaultStyle.info_500_main
										: mainWindow.call.core.encryption === ZuppiEnums.MediaEncryption.Zrtp 
											? mainWindow.call.core.isMismatch || !mainWindow.call.core.tokenVerified
			...
			RowLayout {
				...
				function refreshLayout() {
					if (mainWindow.callState === ZuppiEnums.CallState.Connected || mainWindow.callState === ZuppiEnums.CallState.StreamsRunning
					|| mainWindow.callState === ZuppiEnums.CallState.Paused || mainWindow.callState === ZuppiEnums.CallState.PausedByRemote) {
					...
					else if (mainWindow.callState === ZuppiEnums.CallState.OutgoingInit) {
				...
				Button {
					...
					contentImageColor: DefaultStyle.grey_0
					checkable: false
					Layout.column: mainWindow.callState == ZuppiEnums.CallState.OutgoingInit
										|| mainWindow.callState == ZuppiEnums.CallState.OutgoingProgress
										|| mainWindow.callState == ZuppiEnums.CallState.OutgoingRinging
										|| mainWindow.callState == ZuppiEnums.CallState.OutgoingEarlyMedia
										|| mainWindow.callState == ZuppiEnums.CallState.IncomingReceived
				...
				RowLayout {
					...
					CheckableButton {
						...
						enabled: mainWindow.conference || mainWindow.callState != ZuppiEnums.CallState.PausedByRemote
						icon.source: enabled && checked ? AppIcons.play : AppIcons.phonePause
						checked: mainWindow.call && mainWindow.callState == ZuppiEnums.CallState.Paused || mainWindow.callState == ZuppiEnums.CallState.Pausing || (!mainWindow.conference && mainWindow.callState == ZuppiEnums.CallState.PausedByRemote)
						onClicked: {
				...
				RowLayout {
					...
					CheckableButton {
						...
						enabled: mainWindow.conferenceInfo || (mainWindow.callState === ZuppiEnums.CallState.Connected || mainWindow.callState === ZuppiEnums.CallState.StreamsRunning)
