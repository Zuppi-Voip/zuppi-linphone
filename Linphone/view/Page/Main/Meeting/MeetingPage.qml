...
AbstractMainPage {
	...
	function editConference(confInfoGui = null) {
		...
		var item
		if (isCreation) {
			confInfoGui = Qt.createQmlObject('import Zuppi
											ConferenceInfoGui{
											}', mainItem)
				...
				MeetingForm {
					...
					Connections {
						...
						function onConferenceSchedulerStateChanged() {
							...
							if (meetingSetup.conferenceInfoGui.core.schedulerState == ZuppiEnums.ConferenceSchedulerState.AllocationPending
								|| meetingSetup.conferenceInfoGui.core.schedulerState == ZuppiEnums.ConferenceSchedulerState.Updating) {
								mainWin.showLoadingPopup(qsTr("Création de la réunion en cours..."))
							} else {
								if (meetingSetup.conferenceInfoGui.core.schedulerState == ZuppiEnums.ConferenceSchedulerState.Error) {
									UtilsCpp.showInformationPopup(qsTr("Erreur"), qsTr("La création de la conférence a échoué"), false)
								}
								mainWin.closeLoadingPopup()
							}
							createConfLayout.enabled = meetingSetup.conferenceInfoGui.core.schedulerState != ZuppiEnums.ConferenceSchedulerState.AllocationPending
						}
			...
			ColumnLayout {
				...
				MeetingForm {
					...
					Connections {
						...
						function onConferenceSchedulerStateChanged() {
							var mainWin = UtilsCpp.getMainWindow()
							if (conferenceEdit.conferenceInfoGui.core.schedulerState == ZuppiEnums.ConferenceSchedulerState.AllocationPending
								|| conferenceEdit.conferenceInfoGui.core.schedulerState == ZuppiEnums.ConferenceSchedulerState.Updating) {
								mainWin.showLoadingPopup(qsTr("Modification de la réunion en cours..."))
							} else {
								if (conferenceEdit.conferenceInfoGui.core.schedulerState == ZuppiEnums.ConferenceSchedulerState.Error) {
									UtilsCpp.showInformationPopup(qsTr("Erreur"), qsTr("La modification de la conférence a échoué"), false)
								}
								mainWin.closeLoadingPopup()
							}
							editFocusScope.enabled = conferenceEdit.conferenceInfoGui.core.schedulerState != ZuppiEnums.ConferenceSchedulerState.AllocationPending
						}
						function onSaveFailed() {
							var mainWin = UtilsCpp.getMainWindow()
							mainWin.closeLoadingPopup()
	...
	Component {
		...
		FocusScope{
			...
			ColumnLayout {
					...
					content: RowLayout {
						...
						PopupButton {
							...
							popup.contentItem: Button {
								...
								inversedColors: true
								property var isMeObj: UtilsCpp.isMe(mainItem.selectedConference?.core.organizerAddress)
								property bool canCancel: isMeObj && isMeObj.value && mainItem.selectedConference.core.state !== ZuppiEnums.ConferenceInfoState.Cancelled
				...
				Button {
					id: joinButton
					visible: mainItem.selectedConference && mainItem.selectedConference.core.state !== ZuppiEnums.ConferenceInfoState.Cancelled
					Layout.fillWidth: true
