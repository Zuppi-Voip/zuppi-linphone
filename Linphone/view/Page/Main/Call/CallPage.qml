...
AbstractMainPage {
	...
	property bool isRegistered: account ? account.core?.registrationState == ZuppiEnums.RegistrationState.Ok : false
	...
	Connections {
		...
		function onConferenceSchedulerStateChanged() {
			if (confInfoGui.core.schedulerState === ZuppiEnums.ConferenceSchedulerState.Ready) {
				listStackView.pop()
			}
		}
	}
										...
										RowLayout {
											...
											ColumnLayout {
												...
												RowLayout {
													...
													EffectImage {
														id: statusIcon
														imageSource: modelData.core.status === ZuppiEnums.CallStatus.Declined
														|| modelData.core.status === ZuppiEnums.CallStatus.DeclinedElsewhere
														|| modelData.core.status === ZuppiEnums.CallStatus.Aborted
														|| modelData.core.status === ZuppiEnums.CallStatus.EarlyAborted
															? AppIcons.arrowElbow 
															: modelData.core.isOutgoing
																? AppIcons.arrowUpRight
																: AppIcons.arrowDownLeft
														colorizationColor: modelData.core.status === ZuppiEnums.CallStatus.Declined
														|| modelData.core.status === ZuppiEnums.CallStatus.DeclinedElsewhere
														|| modelData.core.status === ZuppiEnums.CallStatus.Aborted
														|| modelData.core.status === ZuppiEnums.CallStatus.EarlyAborted
														|| modelData.core.status === ZuppiEnums.CallStatus.Missed
														...
														transform: Rotation {
															angle: modelData.core.isOutgoing && (modelData.core.status === ZuppiEnums.CallStatus.Declined
																|| modelData.core.status === ZuppiEnums.CallStatus.DeclinedElsewhere
																|| modelData.core.status === ZuppiEnums.CallStatus.Aborted
																|| modelData.core.status === ZuppiEnums.CallStatus.EarlyAborted) ? 180 : 0
															origin {
					...
					Connections {
						...
						function onStartGroupCallRequested() {
							if (groupCallName.text.length === 0) {
								...
								mainItem.confInfoGui = Qt.createQmlObject('import Zuppi
																			ConferenceInfoGui{
																			}', mainItem)
							...
							RowLayout {
								...
								ColumnLayout {
									...
									RowLayout {
										...
										Text {
											text: modelData.core.status === ZuppiEnums.CallStatus.Missed
												? qsTr("Appel manqué")
												: modelData.core.isOutgoing
													? qsTr("Appel sortant")
													: qsTr("Appel entrant")
									...
									Text {
										text: UtilsCpp.formatDate(modelData.core.date)
										color: modelData.core.status === ZuppiEnums.CallStatus.Missed? DefaultStyle.danger_500main : DefaultStyle.main2_500main
										font {
											pixelSize: 12 * DefaultStyle.dp
											weight: 300 * DefaultStyle.dp
										}
